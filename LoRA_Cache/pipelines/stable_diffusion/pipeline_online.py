    ...
    def __call__(
        ...
        **kwargs,
        ...
        if lora_composite:
            ...
            # first_element = adapters[0]
            # rest_of_list = adapters[1:]
            # random.shuffle(rest_of_list)
            # adapters = [first_element] + rest_of_list
            # adapters = random.shuffle(adapters)
            m = 1
        # mac_list = []
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #         'replicate_prv_feature': prv_features_list[0],
                ...
                if lora_composite and latents is not None:
                    # print(latents.shape)
                    fft = np.mean(latents[0].cpu().numpy(), axis=0)
                    # print(fft.shape)
                    m = cal_mean_relative_log_amplitudes(fft)
                    # print(m.shape)
        ...
        print(f'Interval {cache_interval}: LoRA: {na}')
        # mac = np.mean(mac_list)
        # print("#MACs: {:.4f} G".format(mac/1e9))
